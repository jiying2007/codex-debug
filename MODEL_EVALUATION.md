...
Every reviewed promotion case must contain a unique real historical transition:
...
Promotion Corpus v2 allows one reviewed historical transition to attach an optional `insufficientVariant`. The variant is a second **evaluation view**, not a second reviewed case: it increases `insufficientCases` but never increases the `cases` floor, repository count, failure-kind count, or historical-transition count.

An insufficient variant is fail-closed:

- `expected.assessment = insufficient`;
- `patchPolicy = forbidden`;
- `rootCauseTerms = []`;
- the only allowed projection is digest-bound `{ "version": 1, "mode": "summary-only" }`;
- authors cannot provide custom projected prompt text or hints.

The controller still qualifies the underlying real bad/fix transition with full evidence. During live model evaluation the variant receives only summary-only evidence generated by the controller: failure kind, run/failure/timeout counts and the reproduction-command digest. It receives **no raw stdout/stderr**, source anchors, case identity, reviewed fix SHA, ground-truth summary/files, or Git history; `includeGitHistory=false` is mandatory. A pre-model leakage check fails closed if reviewed ground-truth material appears in the projected evidence.

This allows authentic insufficient-evidence negatives without duplicating one bug to inflate the 12-case floor. The current reviewed corpus intentionally has no insufficient variants yet; schema capability is not itself a quality result.
...
The evaluator records causal-assessment accuracy, root-cause term hit rate, insufficient-evidence accuracy, false support, false-fix candidates, patch-policy violations, both phases' token usage and tokens per evaluation case. Safety-critical counts fail closed in live workflows. Token usage is recorded for calibration; no promotion token-efficiency threshold is claimed yet.

## Claim boundary

Currently allowed statements include: Model Evaluation Record v1 exists; Promotion Corpus v2 and its insufficient-variant projection contract are deterministically tested; a real-historical promotion harness exists; the readiness floor is enforced; historical reproduction/model credentials are separated; four reviewed direct-parent historical cases span Codex Debug, Safe Core and Codex Diagnose; current reviewed coverage is `4/12`, `3/3`, `4/4`, `0/3`; continuous CI proves reviewed Git provenance without executing historical code.
