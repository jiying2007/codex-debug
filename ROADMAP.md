...
- [x] Promotion Model Evaluation sequencing: validate -> governance in promotion mode -> qualify all 12 -> live model -> safety -> Admission v2 -> receipt revalidation -> calibration report -> artifact
...
- [ ] create the required active main Ruleset and merge its administrator-snapshot-derived reviewed Governance Lock
- [ ] explicitly review the separate `promotionEligible=true` corpus change only after calibration evidence is acceptable
- [ ] run `Promotion Model Evaluation` with `promotion_mode=true` and require same-run Governance Receipt + Qualification + live model evidence + `PROMOTION_ADMISSION.json` v2 ready
...
- [ ] Family promotion `development -> active` and immutable release workflow
