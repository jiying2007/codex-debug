...
function calibrationArgs({repository=DEFAULT_REPOSITORY,codexVersion='latest',model='',verifierModel=''}={}){
  return ['workflow','run',WORKFLOW,'--repo',repository,'--ref','main','-f',`codex_version=${codexVersion}`,'-f',`model=${model}`,'-f',`verifier_model=${verifierModel}`,'-f','promotion_mode=false','-f','acknowledge_historical_execution=true'];
}

function parseArgs(argv){
  const out={repository:DEFAULT_REPOSITORY,rulesetName:DEFAULT_RULESET_NAME,requiredCheck:DEFAULT_REQUIRED_CHECK,lockOutput:DEFAULT_LOCK_OUTPUT,codexVersion:'latest',model:'',verifierModel:'',applyRuleset:false,acknowledgeRulesetChange:false,triggerCalibration:false,acknowledgeHistoricalExecution:false};
...
function dryRunPlan(args){
  ...
  return Object.freeze({
    ...
    calibration:{mutationRequires:['--trigger-calibration','--acknowledge-historical-execution'],ghArgs:calibrationArgs(args),promotionMode:false},
...
function triggerCalibration(args,runGh=defaultRunGh){
  ...
  return Object.freeze({triggered:true,workflow:WORKFLOW,promotionMode:false,stdout});
}
...
module.exports={DEFAULT_REPOSITORY,DEFAULT_RULESET_NAME,DEFAULT_REQUIRED_CHECK,DEFAULT_LOCK_OUTPUT,WORKFLOW,rulesetPayload,calibrationArgs,parseArgs,validateArgs,dryRunPlan,listRulesets,fetchRuleset,exactNamedRuleset,ensureRuleset,triggerCalibration,runBootstrap};
