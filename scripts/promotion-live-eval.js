...
function observedEvidence(item,observed){return Object.freeze({text:[`Controller-observed historical failure for reviewed case ${item.id}. Exact reproduction command digest: ${stableDigest(item.reproduction.command)}. Ground-truth fix is intentionally withheld from the model.`,observed.representative.stdout||'',observed.representative.stderr||''].join('\n'),sourceType:'historical-observed',sourceLabel:`reviewed historical case ${item.id}`,includeGitHistory:true});}
function projectedEvidence(item,observed){
  if(!item.insufficientVariant)throw new Error(`missing insufficient variant for ${item.id}`);
  const s=observed.summary||{},runs=Math.max(0,Number(s.runs)||0),failures=Math.max(0,Number(s.failures)||0),timeouts=Math.max(0,Number(s.timeouts)||0);
  return Object.freeze({text:['Controller-observed historical reproduction failed under a reviewed summary-only evidence projection.',`Failure category: ${item.failureKind}.`,`Observed runs: ${runs}; failures: ${failures}; timeouts: ${timeouts}.`,`Exact reproduction command digest: ${stableDigest(item.reproduction.command)}.`,'Raw stdout/stderr, source anchors, Git history, case identity, and the reviewed fix are unavailable in this model-visible evidence.'].join('\n'),sourceType:'historical-projected',sourceLabel:'reviewed historical projected evidence',includeGitHistory:false});
}
function assertProjectedEvidenceBoundary(item,visible){
  if(!item.insufficientVariant)return visible;
  const text=String(visible?.text||'').toLowerCase();
  const forbidden=[item.id,item.insufficientVariant.id,item.fixedCommit,item.groundTruth?.summary,...(item.groundTruth?.files||[])].map(x=>String(x||'').trim().toLowerCase()).filter(Boolean);
  ...
  if(/raw_stdout|raw_stderr/i.test(text))throw new Error(`projected model evidence contains forbidden raw-output marker for ${item.id}`);
  return visible;
}
...
async function runVisibleCase(materialized,item,spec,visible){
  ...
  return resultCase(spec,result,{patchApplicable});
}
async function main(){
  const args=parseArgs(process.argv.slice(2));validatePromotionCorpus(corpus);const readiness=promotionReadiness(corpus);
  ...
  for(const item of corpus.cases){
    ...
    try{
      ...
      cases.push(await runVisibleCase(materialized,item,item,observedEvidence(item,observed)));
      if(item.insufficientVariant){const visible=assertProjectedEvidenceBoundary(item,projectedEvidence(item,observed));cases.push(await runVisibleCase(materialized,item,item.insufficientVariant,visible));}
