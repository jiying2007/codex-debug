...
function verifyImmutableReleaseWorkflow(file){
  ...
  for(const required of ['promotion_run_id','acknowledge_immutable_release','Validate immutable release authority','immutable-release-authority.js','RELEASE_REPOSITORY_GOVERNANCE.json','CI Gate','refs/heads/main','RELEASE_AUTHORITY.json','actions/attest-build-provenance@4d101475d8b20a2381f78447822ac1eab6504dd8','gh release create'])assert.ok(text.includes(required),`immutable release workflow missing ${required}`);
  ...
  return {manualOnly:true,authorityGated:true,publishWriteScoped:true};
}
function verifyDevelopmentBoundary(){
  ...
  for(const file of workflows){
    ...
    for(const forbidden of [/\bpull_request_target\s*:/i,/\bcontents\s*:\s*write\b/i,/\bpackages\s*:\s*write\b/i,/\bid-token\s*:\s*write\b/i,/\bnpm\s+publish\b/i,/\bvsce\s+publish\b/i,/\bovsx\s+publish\b/i,/\bgh\s+release\b/i,/\bgit\s+push\b/i])assert.doesNotMatch(text,forbidden,`non-release workflow contains publication/write surface: ${rel}`);
  }
  ...
  return {lifecycle:contract.lifecycle,workflowCount:workflows.length,immutableReleaseWorkflow:true,publicationSurfaces:false,ordinaryPublicationSurfaces:false};
}
