...
assert.equal(contract.productVersion,'0.1.6','docs gate expects current development identity 0.1.6');
...
for(const term of ['Promotion Corpus v2','summary-only','patchPolicy=forbidden','raw stdout/stderr','Git history','ground-truth summary','ground-truth files','four unique reviewed transitions and three authentic insufficient variants','seven model-evaluation views','3/3','4/12','does **not** prove live-model insufficient-evidence accuracy'])assert.ok(projection.toLowerCase().includes(term.toLowerCase()),`projection security doc missing ${term}`);
for(const implemented of ['Promotion Corpus v2 insufficient-evidence variant contract','three authentic insufficient-evidence variants','4/12','3/3','4/4','0.1.6','Promotion Provenance continues to prove exactly 4 historical transitions'])assert.ok(roadmap.toLowerCase().includes(implemented.toLowerCase()),`ROADMAP missing ${implemented}`);
assert.match(roadmap,/\[x\].*three authentic insufficient-evidence variants/i);
assert.match(roadmap,/\[x\].*package \/ package-lock \/ product-contract development identity.*0\.1\.6/i);
for(const pending of ['add at least 8 more unique reviewed historical transitions','recorded credential-backed live-model RCA corpus','live-model false-fix and patch-applicability benchmark across real repositories','live model token-usage calibration','explicit model behavioral canary where credentials are available'])assert.match(roadmap,new RegExp(`\\[ \\].*${pending.replace(/[.*+?^${}()|[\]\\]/g,'\\$&')}`,'i'));
