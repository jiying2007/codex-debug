...
assert.match(roadmap,/\[x\].*Model Evaluation Record v1.*digest-bound corpus expectations.*false-fix metrics/i);
assert.match(roadmap,/\[x\].*manual read-only live synthetic model canary.*promotionEligible=false/i);
assert.match(roadmap,/Synthetic contract fixtures and synthetic live-canary artifacts are not promotion evidence/i);
...
assert.match(roadmap,/\[ \].*live-model false-fix and patch-applicability benchmark across real repositories/);
assert.match(roadmap,/\[ \].*live model token-usage calibration/);
assert.match(roadmap,/\[ \].*explicit model behavioral canary where credentials are available/);
process.stdout.write(JSON.stringify({lifecycle:contract.lifecycle,docs:['README.md','README.zh-CN.md','ARCHITECTURE.md','SECURITY.md','ROADMAP.md'],status:'current',consumerEvidence:true,executableFixQuality:true,kernelKaslr:true,kernelModuleBuildId:true,rollbackReceiptLineage:true,modelEvaluationV1:true,livePromotionMetrics:false})+'\n');
