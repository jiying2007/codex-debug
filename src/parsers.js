...
const {decodeCortexMFault}=require('./embedded');
const PATTERNS=Object.freeze({
  race:/(?:ThreadSanitizer|data race|race detector|lock-order-inversion)/i,
  sanitizer:/(?:AddressSanitizer|UndefinedBehaviorSanitizer|LeakSanitizer|MemorySanitizer|Valgrind)/i,
  deadlock:/(?:deadlock|lock inversion|blocked for more than|waiting for mutex|mutex .* owner|circular wait)/i,
  oom:/(?:OOMKilled|Out of memory: Kill process|out of memory|Cannot allocate memory|Java heap space|GC overhead limit)/i,
  watchdog:/(?:watchdog:|watchdog timeout|watchdog reset|soft lockup|hard LOCKUP|task watchdog)/i,
  kernel:/(?:kernel panic|\bBUG:|\bOops:|Call Trace:|rcu_sched detected stalls)/i,
...
function extractSignals(text,max=40){const out=[],seen=new Set();for(const raw of String(text||'').split(/\r?\n/)){const line=raw.trim();if(!line)continue;if(/(?:ERROR:|FATAL|panic|Assertion|assertion|race on|heap-use-after-free|stack-buffer-overflow|data race|deadlock|undefined reference|HardFault|BusFault|UsageFault|ANR|SIGSEGV|SIGABRT|watchdog|OOM|out of memory|underrun|overrun|ERLE)/i.test(line)){const normalized=line.replace(/0x[0-9a-f]+/ig,'0xADDR').replace(/\b\d{4,}\b/g,'N');if(!seen.has(normalized)){seen.add(normalized);out.push(line.slice(0,700));if(out.length>=max)break;}}}return freeze(out);}
function parseFailure(text,{kind='auto'}={}){const detectedKind=detectFailureKind(text,kind);const sanitizer=(String(text||'').match(/(?:AddressSanitizer|ThreadSanitizer|UndefinedBehaviorSanitizer|LeakSanitizer|MemorySanitizer|Valgrind)/i)||[])[0]||'';return freeze({kind:detectedKind,sanitizer,frames:extractFrames(text),signals:extractSignals(text),embedded:decodeCortexMFault(text)});}
