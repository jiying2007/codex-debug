...
function platformSymbolRows(value) {
  ...
  if(value.android?.symbolFiles?.length)for(const file of value.android.symbolFiles)out.push(`Android symbols ${file.moduleBase||file.module}: ${file.fileName} digest=${file.fileDigest}`);
  if(value.kernel) {
  ...
  if(value.kernelModules?.symbolFiles?.length)for(const file of value.kernelModules.symbolFiles)out.push(`Kernel module symbols ${file.module}: ${file.fileName} digest=${file.fileDigest}`);
  return out;
}

function rollbackRows(result){
  const rollback=result.lineage?.rollback;
  if(!rollback)return [];
  return [
    `status=completed kind=${rollback.kind}`,
    `snapshot=${rollback.snapshotId}`,
    `paths=${(rollback.paths||[]).join(', ')||'none'}`,
    `rollbackDigest=${rollback.rollbackDigest}`,
    `beforeState=${rollback.beforeStateFingerprint||'unknown'}`,
    `afterState=${rollback.afterStateFingerprint||'unknown'}`
  ];
}
