...
function readCandidate(root,file,maxBytes=64*1024){try{const rootReal=fs.realpathSync.native?fs.realpathSync.native(root):fs.realpathSync(root),absolute=path.resolve(rootReal,file),relative=path.relative(rootReal,absolute);if(relative.startsWith('..')||path.isAbsolute(relative))return '';const stat=fs.lstatSync(absolute);if(stat.isSymbolicLink()||!stat.isFile()||stat.size>maxBytes)return '';const real=fs.realpathSync.native?fs.realpathSync.native(absolute):fs.realpathSync(absolute),realRelative=path.relative(rootReal,real);if(realRelative.startsWith('..')||path.isAbsolute(realRelative))return '';const buffer=fs.readFileSync(real);return buffer.includes(0)?'':buffer.toString('utf8');}catch{return '';}}
