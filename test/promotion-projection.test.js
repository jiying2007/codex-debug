...
test('current reviewed corpus has three authentic insufficient variants without duplicating transitions',()=>{assert.equal(corpus.schemaVersion,2);validatePromotionCorpus(corpus);const readiness=promotionReadiness(corpus);assert.deepEqual(readiness,{ready:false,cases:8,repositories:4,failureKinds:4,insufficientCases:3,gaps:['cases 8/12']});assert.equal(variantCases().length,3);const evalCorpus=toEvaluationCorpus(corpus);assert.equal(evalCorpus.cases.length,11);for(const item of variantCases())assert.ok(evalCorpus.cases.some(x=>x.id===item.insufficientVariant.id));});
...
test('removing variants changes only insufficient readiness, not reviewed case diversity',()=>{const value=clone(corpus);for(const item of value.cases)delete item.insufficientVariant;validatePromotionCorpus(value);const ready=promotionReadiness(value);assert.deepEqual(ready,{ready:false,cases:8,repositories:4,failureKinds:4,insufficientCases:0,gaps:['cases 8/12','insufficientCases 0/3']});});
