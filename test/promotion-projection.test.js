...
test('current reviewed corpus has three authentic insufficient variants while the complete structural floor is satisfied',()=>{assert.equal(corpus.schemaVersion,2);validatePromotionCorpus(corpus);const readiness=promotionReadiness(corpus);assert.deepEqual(readiness,{ready:true,cases:12,repositories:4,failureKinds:5,insufficientCases:3,gaps:[]});assert.equal(corpus.promotionEligible,false);assert.equal(variantCases().length,3);const evalCorpus=toEvaluationCorpus(corpus);assert.equal(evalCorpus.promotionEligible,false);assert.equal(evalCorpus.cases.length,15);for(const item of variantCases())assert.ok(evalCorpus.cases.some(x=>x.id===item.insufficientVariant.id));});
...
test('removing variants changes only insufficient readiness, not reviewed case diversity',()=>{const value=clone(corpus);for(const item of value.cases)delete item.insufficientVariant;validatePromotionCorpus(value);const ready=promotionReadiness(value);assert.deepEqual(ready,{ready:false,cases:12,repositories:4,failureKinds:5,insufficientCases:0,gaps:['insufficientCases 0/3']});});
